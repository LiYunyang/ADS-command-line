...
def scrap_a(fauthor, author_list, year, exact='NO', direct=False):
    ...
    for idx, _ in enumerate(items):
        p = re.compile(
            '(\d*?)</td><td.*?"baseline">(\d*?)\.000.*?"baseline">(\d\d)/(\d{4})(.*?)width="25%">(.*?)<.*?colspan=3>(.*?)</td></tr>',
            re.S)
    ...
    def plot_wordcloud():
        ...
        wordcloud = WordCloud(font_path=font_path, width=1200, height=500, max_words=300, colormap='YlOrRd').generate(titletext)
    ...
    if len(author_list) == 1 and fauthor == '':
        ...
        if year == ['1900', '%d' % int(time.localtime().tm_year)]:
